...
class CBSState:
    ...
    def compute_cost(self):
        ...
        for i in range(self._k):
            # Use A* to find the cost and solution path for each agent
            cost, path = astar.search(self._starts[i], self._goals[i], self._constraints[i])
            total_cost += cost
            # Store the solution path in self._paths
            self._paths[i] = path
    ...
    def is_solution(self):
        ...
        for agent, path in self._paths.items():
            for time_step, state in enumerate(path):
                time_step += state.get_g()
                if (state, time_step) in conflict_set:
                    # Found a conflict, return False and the conflicting state
                    return False, (state, time_step)
                conflict_set.add((state, time_step))

        # No conflicts found, return True and None
        return True, None
    ...
    def successors(self):
        ...
        """
        conflict_exists, conflict_info = self.is_solution()

        if conflict_exists:
            # Extract conflict information
            conflict_state, conflict_time = conflict_info
            # Find the agents involved in the conflict
            agent1, agent2 = None, None
            for agent, path in self._paths.items():
                if conflict_state in path and conflict_state.get_g() <= conflict_time < conflict_state.get_g() + len(path):
                    if agent1 is None:
                        agent1 = agent
                    else:
                        agent2 = agent
...
class CBS():
    def search(self, start):
        ...
        """
        open_list = []
        heapq.heappush(open_list, start)  # Push the initial state (CBSState) into the open_list

        while open_list:
            current_state = heapq.heappop(open_list)

            # Check if current_state is an instance of CBSState
            if not isinstance(current_state, CBSState):
                continue

            is_solution, conflict = current_state.is_solution()
            if is_solution:
                return current_state._paths, current_state.compute_cost()

            successors = current_state.successors()
            for nprime in successors:
                nprime.compute_cost()
                heapq.heappush(open_list, nprime)  # Push the successor (CBSState) into the open_list
